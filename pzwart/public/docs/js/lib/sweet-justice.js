 ...
 *
 * Modified by Yiannis Chatzikonstantinou for
 * use in the Assemblage Plus template in
 * January 2012.
 */

(function($) {

  $.fn.sweet_justice = function() {
    ...
    // There we go...
    var $this = this;
    $this.find('.sweet-justice, .sweet-hyphens').add($this.filter('.sweet-justice, .sweet-hyphens'))
    .each(function(idx,el) {
      justify_my_love(el);
    });
    // Check if copy event is attached. If it is not, attach.
    if (typeof $.data( $('body').get(0), 'events' ) == "undefined" ||
        typeof $.data( $('body').get(0), 'events' ).copy == "undefined") {
